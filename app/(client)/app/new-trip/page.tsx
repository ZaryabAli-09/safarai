...
export default function LuxuryTripPlanner() {
  const [page, setPage] = useState(0);
  const [formData, setFormData] = useState({
    name: "",
    destinations: [] as string[],
  ...
  const handleNext = () => {
    if (page < steps.length - 1) setPage((p) => p + 1);
  };
      ...
      content: (
        <div className="space-y-6">
          <Input
            value={formData.name}
            onChange={(e) => handleChange("name", e.target.value)}
            placeholder="My Dream Escape"
            className="text-lg h-14 border-0 bg-neutral-50 focus-visible:ring-1 focus-visible:ring-neutral-900 placeholder:text-neutral-400"
          />
        </div>
      ),
    },
        ...
        <div className="grid grid-cols-2 gap-3">
          {destinationsList.map((place) => (
            <button
              key={place}
              onClick={() => toggleArrayValue("destinations", place)}
              className={`h-16 rounded-lg transition-all text-sm font-medium ${
                formData.destinations.includes(place)
      ...
      content: (
        <div className="space-y-8">
          <Slider
            ...
            step={5000}
            onValueChange={(val) => handleChange("budget", val[0])}
            className="[&_[role=slider]]:bg-neutral-900 [&_[role=slider]]:border-neutral-900"
          />
          <div className="text-center">
            <p className="text-4xl font-light tracking-tight text-neutral-900">
              {formData.budget.toLocaleString()}
              <span className="text-2xl ml-2 text-neutral-500">PKR</span>
            </p>
          </div>
        </div>
      ),
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              className="h-12 border-0 bg-neutral-50 focus-visible:ring-1 focus-visible:ring-neutral-900"
            />
          </div>
          <div className="space-y-2">
            <label className="text-xs uppercase tracking-wider text-neutral-500">
      ...
      content: (
        <div className="space-y-3">
          <Select
            ...
            onValueChange={(val) => handleChange("tripType", val)}
          >
            <SelectTrigger className="h-12 border-0 bg-neutral-50 focus:ring-1 focus:ring-neutral-900">
            ...
            {steps.map((_, i) => (
              <div
                ...
                className={`h-1 flex-1 mx-0.5 rounded-full transition-all duration-500 ${
                  i <= page ? "bg-neutral-900" : "bg-neutral-200"
                }`}
              />
            ))}
          </div>
          <p className="text-xs text-neutral-400 tracking-wider uppercase text-center">
            Step {page + 1} of {steps.length}
          </p>
        </div>

        {/* Content Card */}
        <div className="bg-white rounded-2xl border border-neutral-200 p-6 md:p-12 shadow-sm">
          {/* Header */}
          ...
          <div className="flex gap-3">
            <Button
              ...
              disabled={page === 0}
              variant="outline"
              className="flex-1 h-12 rounded-lg border-neutral-200 hover:bg-neutral-50 disabled:opacity-30"
            >
            ...
            {page < steps.length - 1 ? (
              <Button
                onClick={handleNext}
                className="flex-1 h-12 rounded-lg bg-neutral-900 hover:bg-neutral-800 text-white"
              >
                Continue
